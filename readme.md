# 👋 Hi, I'm Mahdi — Aspiring Frontend Developer

<img src="https://skillicons.dev/icons?i=html,css,js,react,git,tailwindcss,bootstrap"/>

I'm a self-taught frontend developer passionate about building responsive, user-friendly UIs using modern tools like React and Vite.  
I'm currently looking for a **frontend internship** to grow through real-world experience and team collaboration.

---

## 🔧 Tech Stack

- HTML / CSS / JavaScript  
- React (Hooks, Components, Props)  
- Vite, REST APIs  
- Git & GitHub  
- Mobile-first / Responsive Design  

---

## 📂 Projects

- 🌤️ [Weather App](https://github.com/mahdinap/weather-app) — Real-time weather info using OpenWeatherMap API
- ✅ [To-do List](https://github.com/mahdinap/TodoProApp) — To-do List app save tasks in local storage
- 💲 [Expense Tracker ](https://github.com/mahdinap/expense-tracker) — A simple Expense Tracker app to add, manage, and visualize daily expense


🛠 Coming soon: • Task Manager • News Aggregator • Mini Blog CMS 

---

## 🎯 Goals

- Internship in frontend development  
- Learn TypeScript & Next.js (2025)  
- IELTS 7+ by March 2025  
- Build a strong GitHub portfolio  

---

## 📬 Contact
📧 Email: [mahdi1nap@gmail.com](mailto:mahdi1nap@gmail.com)

<a target="_blank" href="https://instagram.com/mahdi.nap" rel="nofollow">
  <img src="https://img.shields.io/badge/Instagram-%23E4405F.svg?style=for-the-badge&logo=Instagram&logoColor=white" alt="Instagram"/>
</a>
<a href="https://www.linkedin.com/in/mahdi-noorallahpour" rel="nofollow">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
<a href="https://t.me/mahdinap" rel="nofollow">
  <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white" alt="Telegram"/>
</a>

